...
class LogLowestLevelError(Exception):
    def __init__(self, current_level: int, required_level: int):
        """
        현재 logger 객체에 설정된 최소 level이 필수 최소 level보다 
        높을 경우 발생하는 에러. 로깅 되어야 할 메시지가 
        최소 level을 만족시키지 않아 로깅되지 않는 것을 방지하기 위함. 
        예) 
        ...
        매개변수)
        --------
        current_level: 현재 Logger 객체에 설정된 최소 level의 수치값. 
        required_level: 정해진 최소 level의 수치값. 
        """
